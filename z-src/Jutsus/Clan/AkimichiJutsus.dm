		...
		HumanBulletTank()
			for(var/obj/Jutsus/HumanBulletTank/J in src.jutsus)
				if(src.PreJutsu(J))
					...
					if(J.level==1) J.damage=((jutsudamage*J.Sprice)/2)*1.6
					if(J.level==2) J.damage=((jutsudamage*J.Sprice)/1.5)*1.6
					if(J.level==3) J.damage=((jutsudamage*J.Sprice)/1.25)*1.6
					if(J.level==4) J.damage=(jutsudamage*J.Sprice)*1.6
