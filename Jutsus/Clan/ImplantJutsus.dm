	...
	proc
		Sharingan_Copy()
			for(var/obj/Jutsus/Sharingan_Copy/J in src.JutsusLearnt)
				/*if(Sharingan<=2)
					src<<output("You need to have a stronger Sharingan to activate this.","actionoutput")
					return*/
				if(src.PreJutsu(J))
		...
		WarpDim()
			for(var/obj/Jutsus/WarpDim/J in src.JutsusLearnt)
				...
				if(c_target.jailed == 1)
					src<<output("Your target is jailed.","actionoutput")
					return
				if(src.loc.z == 4)
					src<<output("You can't use this in the chuunin exam!","actionoutput")
					return
				if(src.dueling == 1)
					src<<output("You cant use this during a duel!","actionoutput")
					return
				if(src.jailed == 1)
					src<<output("You are jailed.","actionoutput")
					return
				if(src.PreJutsu(J))
		...
		Intangible_Jutsu()
			for(var/obj/Jutsus/Intangible_Jutsu/J in src.JutsusLearnt)
				if(Intang)
					src << output("You're no longer intangible.","actionoutput")
					src.Intang=0
					src.density=1
					spawn(5)
						src.firing=0
					...
					if(jailed==1)
						src<<output("You're not allowed to use intangibility to break out of jail!","actionoutput")
						return
					if(src.loc.z == 3)
						src<<output("You can't use this in the dimension or an arena battle!","actionoutput")
						return

					if(src.PreJutsu(J))
						if(loc.loc:Safe!=1) src.LevelStat("Genjutsu",rand(5,10))
						src << output("You become intangible.","actionoutput")
						src.Intang=1
						src.canattack=0
						src.injutsu=0
						src.firing=0
