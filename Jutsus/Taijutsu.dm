			...
			if(src.client)
				while(src)
					...
					if(PTimes)
						...
						flick("punchl",src)
						view(src) << sound('Sounds/Skill_BigRoketFire.ogg',0,0,0,50)
						sleep(2)
					...
					if(!src.Hit)
							...
							if(c_target<>Owner)
								if(c_target.dead==0)
									if(c_target.fightlayer==src.fightlayer)
										...
										var/undefendedhit=round(((src.damage+Owner.taijutsu+Owner.taijutsu)/3.5)-(c_target.defence/10))
						...
						for(var/obj/Training/T in range(src,1))
							...
							//	if(prob(50))
								...
								//Owner.taiexp++
								spawn()Owner.Levelup()
								//view(src) << sound('Sounds/Skill_MashHit.ogg',0,0,0,10)
								T.Break(Owner)
								break
						sleep(2)
						continue
		...
		Attack()
				...
				if(SC.canattack==1)
					...
					spawn(2)
								...
								if(c_target in get_step(SC,SC.dir))
									if(c_target.dead==0&&!istype(c_target,/mob/NPC/)&&c_target!=SC.Owner)
										if(c_target.fightlayer==SC.fightlayer)
											...
											if(c_target.dodge==0)
												var/undefendedhit=round((2.5+(SC.taijutsu/10))-(c_target.defence/10)+(rand(10)/10))
												if(undefendedhit<0)undefendedhit=1
												c_target.health-=undefendedhit
												var/colour = colour2html("white")
												F_damage(c_target,undefendedhit,colour)
												if(SC.loc.loc:Safe!=1) LevelStat("Taijutsu",rand(1,3))
												...
												if(SC.agility>=c_target.agility)
													var/defendedhit=SC.taijutsu-c_target.defence
													if(defendedhit<0)defendedhit=1
													//WHY IS THIS HERE?-- blame Zetaiva o_o he puts in random stuff like that sometimes if(src.loc.loc:Safe!=1) usr.taijutsu++
													src.Levelup()
													if(c_target.loc.loc:Safe!=1) c_target.LevelStat("Defence",rand(1,2))
													if(defence<SC.taijutsu/3)
						...
						for(var/obj/Training/T in get_step(SC,SC.dir))
							if(T.health>=1)
								var/undefendedhit=round(SC.taijutsu)//-T.defence/4)
								T.health-=undefendedhit
							//	var/colour = colour2html("white")
							//	F_damage(T,undefendedhit,colour)
								if(T) if(T.Good) LevelStat("Taijutsu",rand(1,2))
								else LevelStat("Taijutsu",rand(0.2,1))
								src.Levelup()
								if(SC.Hand=="Left")view(SC,10) << sound('Sounds/LPunchHIt.ogg',0,0,0,100)
								if(SC.Hand=="Right")view(SC,10) << sound('Sounds/HandDam_Normal2.ogg',0,0,0,100)
								T.Break(src)
			...
			if(src.canattack==1&&!src.likeaclone&&!dead&&!rest)
				...
				if(src.icon_state<>"blank")
					...
					if(src.Hand=="Left")
						if(Specialist=="Taijutsu"||Specialist2=="Taijutsu")combo++
						if(src.Clan <> "Sand")
							flick("punchl",src)
							if(src.bugpass)
								var/obj/O = new/obj
							...
							if(src.Clan <> "Sand")
								...
								if(src.bugpass)
									...
									spawn(5)
										if(O)del(O)
										if(O2)del(O2)
							if(Specialist=="Taijutsu"||Specialist2=="Taijutsu")
								src.Hand="Kick"
								combo++
							else src.Hand="Left"
							if(src.Clan == "Sand")
				...
				if(src.agility<50)
					spawn(2)
								...
								if(c_target.dead==0&&!istype(c_target,/mob/NPC/))
									...
									if(c_target.fightlayer==src.fightlayer && canhityou==1)
										...
										if(c_target.dodge==0)
											...
											bonus+=src.bonesword
											var/undefendedhit=(2.5+(src.taijutsu/10))-(c_target.defence/10)+(rand(10)/10)+bonus
											if(undefendedhit<0)undefendedhit=1
											...
											var/colour = colour2html("white")
											F_damage(c_target,undefendedhit,colour)
											if(src.loc.loc:Safe!=1) LevelStat("Taijutsu",rand(1,3))
											if(c_target.loc.loc:Safe!=1) c_target.LevelStat("Defence",rand(1,2))
											c_target.Levelup()
											src.Levelup()
											...
											if(src.agility>=c_target.agility || src.gates[3])
												...
												if(bugpass)bonus+=3
												bonus+=src.bonesword
												var/defendedhit=(1+(src.taijutsu/10))-(c_target.defence/10)+(rand(10)/10)+bonus
												if(defendedhit<0)defendedhit=1
												// WHY WHY WHY ? if(src.loc.loc:Safe!=1) src.taijutsu++
												src.Levelup()
												if(c_target.loc.loc:Safe!=1) c_target.LevelStat("Defence",rand(1,2))
						...
						for(var/obj/Training/T in get_step(src,src.dir))
							if(T.health>=1)
								var/undefendedhit=round(src.taijutsu)//-T.defence/4)
								T.health-=undefendedhit
							//	var/colour = colour2html("white")
							//	F_damage(T,undefendedhit,colour)
								if(T) if(T.Good) LevelStat("Taijutsu",rand(1,2))
									...
									if(c_target.fightlayer==src.fightlayer && canhityou==1)
										...
										if(c_target.dodge==0)
											...
											if(src.agility>=c_target.agility || src.gates[3])
												...
												var/defendedhit=(1+(src.taijutsu/10))-(c_target.defence/10)+(rand(10)/10)+bonus
												if(defendedhit<0)defendedhit=1
												//if(src.loc.loc:Safe!=1) src.taijutsu++
												src.Levelup()
												if(c_target.loc.loc:Safe!=1) c_target.LevelStat("Defence",rand(1,2))
												if(defence<src.taijutsu/3)
													var/obj/Drag=new /obj/Drag/Dirt(c_target.loc)
				...
				if(src.firing==0&&!src.likeaclone)
					for(var/obj/Jutsus/Eight_Trigrams_Empty_Palm/J in src.JutsusLearnt)
						if(J.Excluded==0)
							if(src.chakra>=20)
								J.Excluded=1
								if(loc.loc:Safe!=1) src.LevelStat("Taijutsu",rand(2,5))
								src.Levelup()
								src.UpdateHMB()
								J.uses++
								var/damage
								...
								//src.firing=1
								view(src,10) << sound('Sounds/Skill_BigRoketFire.ogg',0,0,0,100)
								var/obj/O = new/obj
								O.loc = src.loc
								O.dir = src.dir
								...
								for(var/i=1,i<4,i++)
									...
									for(var/mob/c_target in view(O,0))
										...
										if(c_target in view(O,0))
											if(c_target.dead==0&&!istype(c_target,/mob/NPC/))
												if(c_target.fightlayer==src.fightlayer)
													if(c_target.dodge==0)
														var/undefendedhit=round(((damage+src.taijutsu+src.taijutsu)/3)-(c_target.defence/10))
														if(undefendedhit<0)undefendedhit=1
														c_target.health-=undefendedhit
														var/colour = colour2html("white")
														F_damage(c_target,undefendedhit,colour)
														if(loc.loc:Safe!=1) LevelStat("Taijutsu",rand(1,3))
														if(loc.loc:Safe!=1) src.LevelStat("Taijutsu",rand(1,3))
														if(c_target.loc.loc:Safe!=1) c_target.LevelStat("Defence",rand(1,2))
														c_target.Levelup()
														src.Levelup()
														...
														if(src.agility>=c_target.agility)
															var/defendedhit=round(((damage+src.taijutsu+src.taijutsu)/4))
															if(defendedhit<0)
																defendedhit=1
															//if(loc.loc:Safe!=1)src.taijutsu++
															if(loc.loc:Safe!=1)src.LevelStat("Taijutsu",1)
															src.Levelup()
															if(c_target.loc.loc:Safe!=1) c_target.LevelStat("Defence",rand(1,2))
															if(defence<src.taijutsu/3)
																var/obj/Drag=new /obj/Drag/Dirt(c_target.loc)
									...
									for(var/obj/Training/T in view(O,0))
										...
										//	F_damage(T,undefendedhit,colour)
											...
											if(T) if(T.Good) src.LevelStat("Taijutsu",1)
											else src.LevelStat("Taijutsu",0.2)
											src.Levelup()
											if(src.Hand=="Left")view(src,10) << sound('Sounds/LPunchHIt.ogg',0,0,0,100)
											if(src.Hand=="Right")view(src,10) << sound('Sounds/HandDam_Normal2.ogg',0,0,0,100)
											T.Break(src)
					...
					for(var/obj/Jutsus/Leaf_Whirlwind/J in src.JutsusLearnt)
						if(J.Excluded==0)
							if(src.chakra>=20)
								...
								src.firing=1
								flick("spinkick",src)
								if(J.level==3)view(src) << sound('Sounds/wirlwind.ogg',0,0,0,100)
							...
							if(src.chakra>=20)
									...
									if(c_target.dead==0&&!istype(c_target,/mob/NPC/))
										if(c_target.fightlayer==src.fightlayer)
											if(c_target.dodge==0)
												...
												var/undefendedhit=round(((damage+src.taijutsu+src.taijutsu)/6.5)-(c_target.defence/10))
												if(undefendedhit<0)undefendedhit=1
												c_target.health-=undefendedhit
												var/colour = colour2html("white")
												F_damage(c_target,undefendedhit,colour)
												if(loc.loc:Safe!=1)LevelStat("Taijutsu",rand(1,4))
												if(loc.loc:Safe!=1)src.LevelStat("Taijutsu",rand(1,3))
												if(c_target.loc.loc:Safe!=1)c_target.LevelStat("Defence",rand(1,2))
												c_target.Levelup()
												src.Levelup()
												view(src) << sound('Sounds/Skill_MashHit.ogg',0,0,0,50)
												c_target.injutsu=1
												c_target.canattack=0
												c_target.firing=1
												if(c_target.client)spawn()c_target.ScreenShake(1)
												...
												if(src.agility>=c_target.agility)
													var/defendedhit=round(((damage+src.taijutsu+src.taijutsu)/8.5))
													if(defendedhit<0)defendedhit=1
													//if(loc.loc:Safe!=1)src.taijutsu++
													if(loc.loc:Safe!=1)src.LevelStat("Taijutsu",1)
													src.Levelup()
								...
								for(var/obj/Training/T in get_step(src,src.dir))
									...
									//	F_damage(T,undefendedhit,colour)
										...
										if(T) if(T.Good) src.LevelStat("Taijutsu",3)
										else LevelStat("Taijutsu",0.5)
										src.Levelup()
										view(src) << sound('Sounds/Skill_MashHit.ogg',0,0,0,50)
										T.Break(src)
				...
				if(SC.ThrowingMob)
					if(SC.canattack==1&&SC.firing==0)
						if(SC.dead==0&&SC.dodge==0&&SC.canattack==1)
							...
							flick("2fist",SC)
							view(SC) << sound('dash.ogg',0,0,0,100)
							SC.ThrowingMob=null
							M.BeingThrown=null
							M.icon_state="push"
							M.injutsu=1
				...
				if(ThrowingMob)
					if(src.canattack==1&&src.firing==0)
						if(src.dead==0&&src.dodge==0&&src.canattack==1)
							...
							if(!M||M.dead||M.swimming) return
							flick("2fist",src)
							view(src) << sound('dash.ogg',0,0,0,100)
							ThrowingMob=null
							M.BeingThrown=null
							M.icon_state="push"
							M.injutsu=1
						...
						if(M)
							...
							spawn(10)
								if(M)
									...
									M.health-=(70*J.level)+(src.ninjutsu/4)
									F_damage(M,(70*J.level)+(src.ninjutsu/2))
									M.Death(src)
					sleep(1)
				if(I)del(I)
			move=1
				...
				while(rashit==0 && rcount <> 15)
					...
					for(var/mob/M in get_step(src,src.dir))
						if(M)
							...
							M.health-=(40*J.level)+src.ninjutsu/2
							F_damage(M,(40*J.level)+src.ninjutsu/2,colour)
							M.Death(src)
					sleep(1)
				del(I)
			move=1
					...
					while(rashit==0 && rcount <> 15)
						...
						for(var/mob/M in get_step(src,src.dir))
							if(M)
								...
								step(src,src.dir)
								if(M)
									M.health-=(20*J.level)+src.agility
									F_damage(M,(20*J.level)+src.agility)
									if(M)M.Bleed()
									if(M)M.Death(src)
						sleep(1)
					if(src)
					...
					while(rashit==0 && rcount <> 15)
						...
						for(var/mob/M in get_step(src,src.dir))
							if(M)
								...
								step(src,src.dir)
								if(M)
									M.health-=(30*J.level)+src.agility
									F_damage(M,(30*J.level)+src.agility)
									if(M)M.Bleed()
									if(M)M.Death(src)
						sleep(1)
					if(src)
