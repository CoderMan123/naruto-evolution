		...
		Dust_Particle_Prison()
			for(var/obj/Jutsus/Dust_Particle_Prison/J in src.JutsusLearnt)
				if(src.PreJutsu(J))
					var/mob/c_target=src.Target_Get(TARGET_MOB)
					if(!c_target)
						src << output("<Font color=White>You need a target to use this.</Font>","actionoutput")
						return
					if(loc.loc:Safe!=1) src.LevelStat("Ninjutsu",rand(4,8))
					src.canattack=0
					src.injutsu=1
			...
			for(var/obj/Jutsus/Dust_Particle_Prison_Beam/J in src.JutsusLearnt)
				if(src.PreJutsu(J))
					...
					if(c_target.industprison!=1)
						src << output("<Font color=White>Your target needs to be in Dust Particle Prison in order for jutsu to work..</Font>","actionoutput")
						return
					if(loc.loc:Safe!=1) src.LevelStat("Ninjutsu",rand(4,8))
					src.injutsu=1
					src.firing=1
