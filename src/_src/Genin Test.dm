...
proc/GeninExam()
	while(world)
		...
		for(var/mob/player/M in mobs_online)M<<output("<center>A Genin exam is about to begin in all Village Academies.</center>","Action.Output")
		sleep(600*3)
		for(var/mob/player/M in mobs_online)M<<output("<center>The Genin exam has begun!</center>","Action.Output")
		GeninTest=1
		sleep(600*2)
		for(var/mob/player/M in mobs_online)M<<output("<center>The written Genin exam is now over! Handseals testing will begin in one minute for all who passed!</center>","Action.Output")
		GeninTest=0
		sleep(100)
		...
		genintesters=list()
		for(var/mob/player/M in mobs_online)M<<output("<center>The practical examination for the Genin Exams are now over. Thank you for those that participated!</center>","Action.Output")
mob
	proc/givegenin()
