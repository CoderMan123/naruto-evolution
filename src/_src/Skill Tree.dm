...
mob
	verb
		viewskilltree()
			set hidden=1
			if(Tutorial<3)
				src<<output("You cannot do this right now.","Action.Output")
				return
			...
			if(client.eye==locate(10,10,4)||client.eye==locate(60,10,4)||client.eye==locate(9,124,4)||client.eye==locate(12,43,4)||client.eye==locate(55,43,4)||usr.client.eye==locate(10,75,4)||usr.client.eye==locate(37,75,4))
				...
				if(src.village == "Hidden Mist")new/obj/Screen/MistSymbol(src)
				if(src.village == "Hidden Sound")new/obj/Screen/SoundSymbol(src)
				if(src.village == "Hidden Rock")new/obj/Screen/RockSymbol(src)
		...
		updateskills()
			...
			for(var/obj/Jutsus/O in world) if(O.z==4)
				...
				if(O.name in usr.sbought)I.overlays+=image('Misc Effects.dmi',"hasit!")
					...
					if(O.Specialist)if(O.Specialist!=usr.Specialist)SpecialistZ=1
					if(Element1)has_reqs=0
					if(Element2z)has_reqs=0
					if(SpecialistZ)has_reqs=0
