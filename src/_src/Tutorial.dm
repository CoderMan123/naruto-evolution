	...
	Tutorial
		...
		//	var/message
			...
			DblClick()
				...
				if(get_dist(src,usr)>2)return
				if(usr)usr.move=0
				if(usr) usr.client.Alert({"Hey there, I am the Ninjutsu trainer. I'll teach you how to learn new jutsu as well as some basic techniques that even the strongest of ninja use!"},"[src]")
