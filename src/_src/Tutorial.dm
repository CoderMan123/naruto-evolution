	...
	Tutorial
		...
		Starting_Jounin
			...
			DblClick()
				...
				if(usr)
					usr.client.Alert(
						{"[src]: Hey there, welcome to Naruto Evolution: The New Era. Please take a moment to complete the tutorial.
						It will help you better understand how to play as well as where to find everything. "}, "[src]")

				if(usr)
					usr.client.Alert(
						{"[src]: First, you should have a look at the interface. The \"Chat\" button at the bottom left will toggle your chatbox. Next to it is the \"Channel\" button.
				...
				if(usr)
					usr.client.Alert(
						{"[src]:Last but not least is the Options button. In there you will find many buttons with unique outcomes, most of these are pretty self explanitory.
				...
				if(usr)
					usr.client.Alert(
						{"[src]: Your health and chakra bar is located at the top left of the screen, if your jutsus are on cooldown they'll appear to the right of the screen.
			...
			DblClick()
				...
				if(usr.strength<=usr.TutorialStrength)
					if(usr) usr.client.Alert(
						{"[src]: Hey, talk to me again when you've increased your Taijutsu.
				...
				if(usr)usr.move=0
				if(usr) usr.client.Alert(
					{"[src]: Welcome back. I see you leveled your Taijutsu. This means you already know the basics of leveling stats. Punch things with A, get taijutsu experience.
				...
				if(usr) usr.client.Alert({"[src]: Another thing that you should note is targetting. You can target other players by clicking on them or by pressing Tab to cycle through targets.
...
You'll learn more about this as you experience it first hand. Please move on to the next Jounin when you're ready."},"[src]")
		...
		//	var/message
			...
			DblClick()
				...
				if(get_dist(src,usr)>2)return
				if(usr)usr.move=0
				if(usr) usr.client.Alert({"[src]: Hey there, I am the Ninjutsu trainer. I'll be explaining how the art of Ninjutsu works. To many, Ninjutsu is one of the most vital
...
Using the keys displayed next to the technique, you can preform the jutsu, and execute it, by pressing the spacebar."},"[src]")
				if(usr) usr.client.Alert({"[src]: Eventually, you will be able to macro these techniques and be able to add them to your hotslot bar.
...
If the jutsu doesn't have handsigns you can drag it to the hotbar(F keys at bottom of screen) without using it 80 times."},"[src]")
				if(usr) usr.client.Alert({"[src]:  Another important factor in training Ninjutsu is the levels each technique has.
...
Some jutsus increase damage or active time while others will fire more projectiles. Substitution even grants Advanced substitution when leveled enough. Try them out for yourself."},"[src]")
		...
		//	var/message
			DblClick()
				...
				if(usr) usr.client.Alert({"[src]: Hi. I'm the final Jounin instructor. I'll be briefing you on the some of the other systems you can find here. "},"[src]")
				if(usr) usr.client.Alert({"[src]: Training can be done in many ways. Go swimming, use chakra control to walk on water, use techniques by yourself in the dojo where it's safe, do missions with fellow villagers for experience, or even both.
If you're out and about training you have to be weary of other ninjas, expecially from other villages. They may try to kill you, that is, until you get stronger and kill them first! Hahaha. "},"[src]")
				if(usr) usr.client.Alert({"[src]: To survive to the fullest extent you must cooperate with your fellow ninja to defend yourselves, eachother, and the village.
Not to mention doing missions in a squad yields experience to the entire squad.
Teamwork makes the dream work."},"[src]")
				if(usr) usr.client.Alert({"[src]: Don't worry, the tutorial is almost over. Go take a mission picking weeds from Hisho in the Kage house.
Then train your Taijutsu, Ninjutsu, and Genjutsu to 5, by using the jutsus you already have, so you can easily take down a rogue shinobi for the key to experiencing the rest of the world."},"[src]")
				if(usr) usr.client.Alert({"[src]: Don't bother remembering the tasks ahead. Start with the weeding mission and we'll remind you what's next as you go.
