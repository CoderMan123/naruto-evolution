	...
	Kage
		verb
			PeaceTreaty()
				...
				if(!(VillageDefenders.Find(usr.village)) && !(VillageAttackers.Find(usr.village))) {usr<<output("<br><center><b>Your Village isn't in a war!</b><br>","ActionPanel.Output");return}
				...
				if((VillageDefenders.Find(input1)) && (VillageAttackers.Find(usr.village))||(VillageAttackers.Find(input1)) && (VillageDefenders.Find(usr.village)))
					...
					for(var/mob/M in mobs_online)
						if(M.client && findtext("[M.rank]","kage",1) && M.village=="[input1]")
							...
							if(input2=="Yes")
								world<<output("<br><b><center>[input1] Village has made peace with [usr.village] Village!</b><br>","ActionPanel.Output")
								if("[input1]" in VillageDefenders)
									VillageAttackers-="[Villagetochange]"
									VillageDefenders-="[input1]"
									if(!VillageAttackers.len)
										world<<output("<br><b><center>All wars have ended!</font><br>","ActionPanel.Output")
							...
							else usr<<output("<br><center>[input1] has declined to the peace offering!<br>","ActionPanel.Output")
					if(!found)usr<<output("<br><center>Their kage is not online at this moment<br>","ActionPanel.Output")
				else usr<<output("<br>You're not at war with them.<br>","ActionPanel.Output")
				usr.choosing=0
			DeclareWar()
				set category = "Kage"
				set name = "Declare War"
				if(!(VillagesInGame.Find(usr.village))) return
				if(VillageDefenders.Find(usr.village)) {usr<<output("<br><center><b>Your village is already in a war!</b><br>","ActionPanel.Output");return}
				if(VillageAttackers.Find(usr.village)) {usr<<output("<br><center><b>Your village is already in a war!</b><br>","ActionPanel.Output");return}
				if(usr.choosing) return
				usr.choosing=1
				var/tmp/list/warVillages=list("Hidden Leaf","Hidden Sand","Hidden Mist","Hidden Sound","Hidden Rock","Akatsuki","Seven Swordsmen","Anbu Root")
				...
				if(VillageDefenders.Find(input1))
					...
					if(input2=="Attack")
						...
						world<<output("[msg]","ActionPanel.Output")
					else
						var/msg="<center><b>[usr.village] has joined the War to help"
						var/amount=0
