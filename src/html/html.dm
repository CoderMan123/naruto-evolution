...
proc/HTML_GetCharacter(mob/m)
    if(m.client)
        ...
        if(!m.village)
            return "<font color='[COLOR_VILLAGE_MISSING_NIN]'>[m.character]</font>"
        else
            switch(m.village)
                if(VILLAGE_LEAF)
                    return "[badges] <font color='[COLOR_VILLAGE_LEAF]'>[m.character]</font>"

                if(VILLAGE_SAND)
                    return "[badges] <font color='[COLOR_VILLAGE_SAND]'>[m.character]</font>"

                if(VILLAGE_MISSING_NIN)
                    return "[badges] <font color='[COLOR_VILLAGE_MISSING_NIN]'>[m.character]</font>"

                if(VILLAGE_AKATSUKI)
                    return "[badges] <font color='[COLOR_VILLAGE_AKATSUKI]'>[m.character]</font>"
