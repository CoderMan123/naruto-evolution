...
mob/verb/
	Vote_Boot()
		...
		if(count>=1 && !VoteB) // This way there needs to be two votes to mute initially.
			...
			if(src.RecentVerbsCheck("VoteCoolDownBoot",12000,1))src<<output("You can only do a boot vote once every 20 minutes.","actionoutput")
...
proc/VoteB(mob/Who)
	...
	while(TimeLeft&&!Who.MuteTime)
		...
		TimeLeft--
		if(VotedYes.len>VotedNo.len&&VotedYes.len>NonVoters)
			world<<"Vote passed, [Who] has been booted."
			//var/WhoKey=Who.key
